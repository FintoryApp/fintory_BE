package com.fintory.auth.service.socialuserservice.parentOAuth;
...
public class ParentGoogleOauthService {
    ...
    public AuthToken handleGoogleLoginOrRegister(String idToken) {
        ...
        //idpw로 가입되어있는지 검사
        Optional<Parent> check = parentRepository.findByEmail(googleEmail);
        ...
        Parent parent = parentRepository.findBySocialId(socialId) // 소셜로만 가입되있는 경우 -> 로그인
                .orElseGet(() -> { // 소셜로그인 가입 안되어있는 경우 -> 등록 후 로그인
                    Parent newParent = Parent.createWithSocial(
                            nickname, googleEmail, socialId, LoginType.GOOGLE, Role.PARENT, Status.ACTIVE
                    );
                    return parentRepository.save(newParent);
                });
        // 인증 객체 생성 (비밀번호 없이)
        CustomUserDetails userDetails = new CustomUserDetails(
                parent.getEmail(),  // username
                null,                 // password: 소셜 로그인은 비밀번호 불필요
                parent.getNickname(),
                parent.getRole().getKey(),
                parent.getLoginType()
        );
