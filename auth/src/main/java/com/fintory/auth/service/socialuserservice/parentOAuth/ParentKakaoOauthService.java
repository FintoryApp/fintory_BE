...
public class ParentKakaoOauthService {
    ...
    public AuthToken handleKakaoLoginOrRegister(String kakaoAccessToken) {
        ...
        // id, pw로 가입되어있는지 검사
        Optional<Parent> check = parentRepository.findByEmail(kakaoEmail);
        ...
        Parent parent = parentRepository.findBySocialId(kakaoId)
                .orElseGet(() -> {
                    Parent newParent = Parent.createWithSocial(
                            nickname, kakaoEmail, kakaoId, LoginType.KAKAO, Role.PARENT, Status.ACTIVE
                    );
